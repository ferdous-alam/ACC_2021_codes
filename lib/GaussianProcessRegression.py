...
class GaussianProcess:
    ...
    def plotGPcov(self):
        ...
        ax.scatter3D(self.X[:, 0], self.X[:, 1], self.Y, c=self.Y,
                     label='training data', alpha=0.25)
        plt.show()
    ...
    def plotGPstochastic(self, meanVal):
        ...
        ax.set_xlabel(r'filament distance $(\mu m)$', fontsize=30)
        ax.set_ylabel(r'filament diameter $(\mu m)$', fontsize=30)
        ax.set_zlabel(r'loss', fontsize=30)
